...
1. **Provision a Supabase project** — needed before Phase 1 can integrate auth and migrations.
2. **Start Phase 1** — read [PHASE-1-foundation.md](../phases/PHASE-1-foundation.md) in full, propose a plan, then implement.
3. **Establish `.env.example`** with the first real variables (Supabase URL, anon key, service-role key, app URL).
